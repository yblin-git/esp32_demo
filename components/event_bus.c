 ...
 * @LastEditors: yblin
 * @LastEditTime: 2022-04-22 11:42:19
 * @Description: 
 */
#include "esp_log.h"
...
esp_err_t evnet_bus_register(int32_t event_id, esp_event_handler_t event_handler){
    ...
    return esp_event_handler_instance_register_with(event_bus_handle, APP_EVENTS, event_id, event_handler,event_bus_handle, NULL);
}
...
esp_err_t evnet_bus_send(int32_t event_id,
    ...
    return esp_event_post_to(event_bus_handle, APP_EVENTS, event_id, event_data, event_data_size, ticks_to_wait);
}

esp_err_t evnet_bus_isr_send(int32_t event_id,
        const void* event_data, size_t event_data_size, BaseType_t *task_unblocked){
    ESP_LOGI(TAG, "sevnet_bus_send");
    return esp_event_isr_post_to(event_bus_handle, APP_EVENTS, event_id, event_data, event_data_size, task_unblocked);
}